...
var LoggedIn = React.createClass({
  logout : function(){
    ...
    this.props.lock.logout({returnTo:'http://localhost:8080'})
  },
  ...
  componentDidMount: function() {
    ...
    this.serverRequest = $.get('http://localhost:8080/todos', function (result) {
      console.log(result)
      this.setState({
        products: result,
      });
    ...
    if (this.state.profile) {
      return (
        <div className="col-lg-12">
          ...
          <span className="pull-left">{localStorage.getItem('userToken')} <a onClick={this.logout}>Log out</a></span>
...
var Product = React.createClass({
  upvote : function(){
    ...
    this.serverRequest = $.post('http://localhost:8080/products/' + product.Slug + '/feedback', {vote : 1}, function (result) {
      this.setState({voted: "Upvoted"})
    }.bind(this));
    // this.serverRequest = $.get('http://localhost:8080/todos'
  },
  downvote: function(){
    var product = this.props.product;
    this.serverRequest = $.post('http://localhost:8080/products/' + product.Slug + '/feedback', {vote : -1}, function (result) {
